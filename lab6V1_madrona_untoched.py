...
# Others: https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.axis.html (1.5%)
#         https://www.w3schools.com/python/ref_string_format.asp                (1.5%)
...
# occultSquare() Function:
# accepts two arguments: im (array) and width (int)
# creates an black square of specified width at the center of the image
# by turning all RGB of the affected rows and columns into zero
# returns the resulting image
def occultSquare(im,width):
    imS = im.copy()
    
    xstart = int((imS.shape[1]-width)/2) # column starting point (and endpoint when you put (-) sign) to turn all pixel into black (0)
    ystart = int((imS.shape[0]-width)/2) # row starting point (and endpoint when you put (-) sign) to turn all pixel into black (0)

    imS[ystart:-ystart, xstart:-xstart] = 0
